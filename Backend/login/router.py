...
from login.models import Token, UserLogin
from login.service import authenticate_user, create_access_token, ACCESS_TOKEN_EXPIRE_MINUTES, get_current_user
from jose import jwt, JWTError

# Import JWT configuration
from login.service import SECRET_KEY, ALGORITHM
...
def login_for_access_token(user_data: UserLogin, db: Session = Depends(get_db)):
    ...
    This endpoint is used by the React frontend application.
...
def verify_token(token: str, db: Session = Depends(get_db)):
    ...
    - **token**: Your JWT token (without 'Bearer ' prefix)
    
    Returns detailed information about the token and the user if valid.
    """
    ...
    try:
        ...
        response["error"] = f"Invalid token format: {str(e)}"
